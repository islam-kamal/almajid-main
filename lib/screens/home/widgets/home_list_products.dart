      ...
      case 'related products':
        widget.items!.productLinks!.forEach((element) async {
          if (element.linkType == "related") {
            var response = await categoryRepository.getProduct(
                sku: element.linkedProductSku);
            related_product_list
                .add(response.items!.isNotEmpty ? response.items![0] : null!);
  ...
  Future<void> readJson() async {
    if (StaticData.data != null) {
      home_bloc.add(GetHomeNewArrivals(
          category_id: StaticData.data['new-arrival']['id'], offset: 1));
      home_bloc.add(GetHomeBestSeller(
          category_id: StaticData.data['best-seller']['id'], offset: 1));
      home_bloc.add(GetWeeklyDealSeller(
          category_id: StaticData.data['weekly-deal']['id'], offset: 1));
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data!.isEmpty) {
              return no_data_widget(context: context, image_status: true);
            } else {
                  ...
                  child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        snapshot.data![index] == null
                            ? null
                            : snapshot.data![index].customAttributes!
                                .forEach((element) {
                                if (element.attributeCode == "thumbnail")
                                  product_image = element.value;
                                else if (element.attributeCode ==
                                    "special_from_date") {
                                  startDate = DateTime.parse(element.value
                                      .toString()
                                      .substring(0, 10));
                                } else if (element.attributeCode ==
                                    "special_to_date") {
                                  endDate = DateTime.parse(element.value
                                      .toString()
                                      .substring(0, 10));
                                } else if (element.attributeCode ==
                                    'special_price') {
                        ...
                        if (startDate == null || endDate == null) {
                          ...
                          if (double.parse(minimal_price) <
                              double.parse(
                                  snapshot.data![index].price.toString()))
                            percentage = (1 -
                                    (double.parse(minimal_price) /
                                        snapshot.data![index].price)) *
                                100;
                        } else {
                          if (StaticData.isCurrentDateInRange(
                                  startDate!, endDate!) &&
                              double.parse(special_price!) <=
                                  double.parse(minimal_price) &&
                            ...
                            percentage = (1 -
                                    (double.parse(new_price) /
                                        snapshot.data![index].price)) *
                                100;
                          } else if (double.parse(special_price!) >
                              double.parse(minimal_price)) {
                            new_price = minimal_price;
                            percentage = (1 -
                                    (double.parse(new_price) /
                                      ...
                                      snapshot.data![index].extensionAttributes!.stockQty >= 1
                                          ?   percentage == null || percentage == 0
                                              ...
                                              : Row(
                                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                  children: [
                                                    percentage == null || percentage.isNaN || percentage.isInfinite
                                                        ? Container()
                                                        : Container(
